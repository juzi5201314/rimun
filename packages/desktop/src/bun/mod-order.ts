...
function buildTopologicalOrder(
  ...
  // 稳定拓扑排序：尽量保持当前 active 顺序，同时满足硬约束。
...
export function analyzeModOrderFromSelection(
  ...
  const modLibrary = scanModLibrary(selection, {
    environment,
    toReadablePath,
  });
...
export function applyModOrderRecommendation(
  selection: PathSelection | null,
  ...
  if (input.actions.includes("reorderActiveMods")) {
    if (analysis.hasBlockingIssues) {
      throw new Error("Cannot reorder mods while blocking issues remain.");
    }

    const targetActivePackageIds = input.actions.includes(
  ...
  const nextModLibrary = scanModLibrary(selection, {
    environment,
    toReadablePath,
  });
  const nextAnalysis = analyzeModOrder(nextModLibrary);

  return {
    appliedActions: input.actions,
    activePackageIds: nextModLibrary.activePackageIds,
    modLibrary: nextModLibrary,
